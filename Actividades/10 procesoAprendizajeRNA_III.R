#Proceso de aprendizaje en redes neuronales
...
#Normalizar los datos con escalamiento de características
...
#Construir la red neuronal - Una capa oculta con 10 nodos
n = names(data) 
f = as.formula(paste("medv ~", paste(n[!n %in% "medv"], collapse = " + "))) 

#net_data = neuralnet(f,data=train_data,hidden=10,linear.output=T)
#net_data = neuralnet(f,data=train_data,hidden=10,5,linear.output=T)
#net_data = neuralnet(f, data=train_data, hidden=c(10, 5), linear.output=T)
net_data = neuralnet(f, data=train_data, hidden=c(10, 10, 10), linear.output=T)

plot(net_data) 

#Realizar predicciones - Restaurar la normalización para el MSE
...
#Construir un modelo de regresión lineal para comparar
...
#Comparar MSE de la RNA y el modelo de reg.
MSE.net_data 
MSE.lm 
